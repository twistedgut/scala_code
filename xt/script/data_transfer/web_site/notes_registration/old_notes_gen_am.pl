...
foreach my $id (@$notes_ids){
    ...
    $sth = $dbh->prepare("
    SELECT np.product_id, d.designer, np.product_name, round(np.price_gbp) as price_gbp, round(np.price_eur) as price_eur, round(np.price_usd) as price_usd, p.live, p.visible, CASE WHEN d.designer LIKE '3.1 %' THEN replace(d.designer, '3.1 ', '') WHEN d.designer ~ 'Chlo' THEN substring(d.designer, '^(.*Chlo)')||'e' ELSE d.designer END as sort_by_designer
      FROM notes_product np, product p, designer d
...
EOT
    ...
    #$so .= "<option value='$designer1'>$p->{designer}</option>\n";        
        ...
        elsif ($p->{price_gbp} && $p->{price_eur}==0) {
            $price_str_gbp_eur = "\&pound;$p->{price_gbp} (\$TBC)";
        }
        else {
            $price_str_gbp_eur = "\&pound;$p->{price_gbp} (\$&euro;$p->{price_eur})";
        }
...
<td align=right>$price_str_usd</td>
<td>&nbsp;</td>
EOL
        if( $p->{live} && $p->{visible} ) {
