...
my $dbh_dc2 = get_database_handle( { name => 'XTracker_DC2', type => 'readonly' } );
...
# get DC2 product visibility data
my %prod_data = ();
$sth = $dbh_dc2->prepare("select product_id, live, visible from product_channel where channel_id = 2");
$sth->execute();
...
EOL
        ...
        # push dc2 prod data into live and visible values
        $p->{live} = $prod_data{ $p->{product_id} }{live};
        $p->{visible} = $prod_data{ $p->{product_id} }{visible};
