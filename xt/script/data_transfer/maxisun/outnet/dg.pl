...
open my $fh, '>', '/var/data/xt_static/data/maxisun/'.$outdir.'/dg.csv' || die "Couldn't open output file: $!";
...
union
select distinct del.id as delivery_id, to_char(del.date, 'DDMMYYYY') as date, 'none' as description, pri.wholesale_price, cur.currency, p.legacy_sku, d.designer, s.season, ctry.code as countryoforigin, po.purchase_order_number, po.type_id, p.style_number, p.designer_id, p.season_id, po.id as purchase_order_id, sup.code, sup.description as sup_description
from purchase_order po, stock_order so, stock_order_item soi, delivery del,
log_delivery ld, product p, season s, price_purchase pri, currency cur,
designer d, product_attribute att, variant v, legacy_designer_supplier lds,
supplier sup, shipping_attribute sa LEFT JOIN country ctry ON sa.country_id = ctry.id, link_delivery__stock_order ld_so
where ld.date between ? and ?
and ld.delivery_action_id = 6
and ld.delivery_id = del.id
and del.status_id > 1
and po.type_id = 3
and po.id = so.purchase_order_id
...
and p.id = sa.product_id
and po.channel_id = ?
group by del.id, del.date, pri.wholesale_price, cur.currency, p.legacy_sku,
d.designer, s.season, ctry.code, po.purchase_order_number,
p.style_number, p.designer_id, p.season_id, po.id, sup.code, sup.description,
po.type_id
";
...
foreach my $record ( @purchase_order ){
    ...
    print $fh "$record->{arrival_id}~$record->{date}~~$record->{supplier}~$record->{currency}~$record->{supplier_code}~$amount~$total_quantity~$record->{sku}~$record->{designer_id}~$record->{season_id}~$record->{country_code}~~~~$po~~~$record->{type}\r\n";
}
