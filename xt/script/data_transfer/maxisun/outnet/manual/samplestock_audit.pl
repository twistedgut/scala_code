...
# open output file
open my $fh, '>', '/var/data/xt_static/data/maxisun/'.$outdir.'/SAMPLESTOCK_AUDIT.csv' || die "Couldn't open output file: $!"; 

my $qry = "
select to_char(current_timestamp, 'DDMMYYYY') as date, p.id as product_id, p.legacy_sku, p.style_number, s.code, pri.uk_landed_cost, sum(q.quantity) as quantity, p.designer_id, p.season_id, sa.legacy_countryoforigin, l.location
from variant v 
    left join quantity q on v.id = q.variant_id and q.location_id in (select id from location where type_id in (4,6)),  
product p 
...
and q.channel_id = ? 
group by p.id, p.legacy_sku, p.style_number, s.code, pri.uk_landed_cost, p.designer_id, p.season_id, sa.legacy_countryoforigin, l.location
";
