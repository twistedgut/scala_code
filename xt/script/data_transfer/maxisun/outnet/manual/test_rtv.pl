...
my $qry = "
SELECT rsd.rtv_shipment_id, rsd.rma_request_detail_id, sum(rsd.quantity) as quantity, rrdt.type, to_char(log.date_time, 'DDMMYYYY') as date, p.legacy_sku, p.designer_id, p.season_id, c.country as country_of_origin, sup.code as supplier_code, sup.description as supplier_name, pri.wholesale_price, cur.currency, po.purchase_order_number
FROM rtv_shipment_detail_status_log log, rtv_shipment_detail rsd, rma_request_detail rrd, rma_request_detail_type rrdt, variant v, product p, legacy_designer_supplier lds, supplier sup, shipping_attribute sa, country c, price_purchase pri, currency cur, stock_order so, purchase_order po
...
AND po.type_id = 1
GROUP BY rsd.rtv_shipment_id, rsd.rma_request_detail_id, rrdt.type, to_char(log.date_time, 'DDMMYYYY'), p.legacy_sku, p.designer_id, p.season_id, c.country, sup.code, sup.description, pri.wholesale_price, cur.currency, po.purchase_order_number
";
my $sth = $dbh->prepare($qry);
$sth->execute($start, $end);

open my $fh, ">", "/var/data/xt_static/utilities/data_transfer/maxisun/intl/csv/test_rtv.csv" || die "Couldn't open file: $!";
...
while( my $record = $sth->fetchrow_hashref() ){
   ...
   print $fh "$record->{rtv_shipment_id}~";
