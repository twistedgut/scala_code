...
v.legacy_sku, v.product_id, v.product_id || '-' || sku_padding(v.size_id) as sku, p.id, p.style_number, s.code, pri.uk_landed_cost, sum(q.quantity) as
quantity, p.designer_id, p.season_id, sa.legacy_countryoforigin, l.location
from variant v 
left join quantity q on v.id = q.variant_id and q.channel_id = 2 
and q.location_id not in (select id from location where type_id in (2, 3, 4, 6) and location != 'Transfer Pending')
...
open my $fh, ">", "/var/data/xt_static/utilities/data_transfer/maxisun/am/csv/MAINSTOCK_AUDIT.csv" || die "Couldn't open file: $!";
...
while ( my $row = $sth->fetchrow_hashref() ) {
    ...
    if ($goodsin{$row->{var_id}} && !$got{$row->{var_id}}{goodsin}) {
        ...
        foreach my $status_id (keys %{$goodsin{$row->{var_id}}} ) {
            ...
            print $fh "$row->{legacy_sku},$row->{product_id},$row->{sku},$row->{date},,$row->{style_number},,USD,$row->{code},$cost,$goodsin{$row->{var_id}}{$status_id}{qty},$row->{legacy_sku},$row->{designer_id},$row->{season_id},GOODS IN - $status\n";
    ...
    if ($faulty{$row->{var_id}} && !$got{$row->{var_id}}{faulty}) {
        ...
        print $fh "$row->{legacy_sku},$row->{product_id},$row->{sku},$row->{date},,$row->{style_number},,USD,$row->{code},$cost,$faulty{$row->{var_id}},$row->{legacy_sku},$row->{designer_id},$row->{season_id},GOODS IN - FAULTY\n";
    ...
    if ($rtv{$row->{var_id}} && !$got{$row->{var_id}}{rtv}) {
        ...
        print $fh "$row->{legacy_sku},$row->{product_id},$row->{sku},$row->{date},,$row->{style_number},,USD,$row->{code},$cost,$rtv{$row->{var_id}},$row->{legacy_sku},$row->{designer_id},$row->{season_id},GOODS IN - RTV\n";
    ...
    if ($returns{$row->{var_id}} && !$got{$row->{var_id}}{returns}) {
        ...
        foreach my $status_id (keys %{$returns{$row->{var_id}}} ) {
            ...
            print $fh "$row->{sku},";
            print $fh "$row->{date},,";
            print $fh "$row->{style_number},,USD,";
            print $fh "$row->{code},";
            print $fh "$cost,";
            print $fh "$returns{$row->{var_id}}{$status_id}{qty},";
            print $fh "$row->{legacy_sku},";
            print $fh "$row->{designer_id},";
            print $fh "$row->{season_id},";
            print $fh "RETURNS IN - $status\n";
    ...
    if ($picked{$row->{var_id}} && !$got{$row->{var_id}}{picking}) {
        ...
        print $fh "$row->{legacy_sku},$row->{product_id},$row->{sku},$row->{date},,$row->{style_number},,USD,$row->{code},$cost,$picked{$row->{var_id}},$row->{legacy_sku},$row->{designer_id},$row->{season_id},PICKED\n";
    }
    ...
    if ($rtv_picked{$row->{var_id}} && !$got{$row->{var_id}}{rtv_picking}) {
        ...
        print $fh "$row->{legacy_sku},$row->{product_id},$row->{sku},$row->{date},,$row->{style_number},,USD,$row->{code},$cost,$rtv_picked{$row->{var_id}},$row->{legacy_sku},$row->{designer_id},$row->{season_id},RTV Picked\n";
    }
