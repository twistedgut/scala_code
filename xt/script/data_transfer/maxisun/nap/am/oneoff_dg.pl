...
#my $qry = "
#select a.arrival_id, to_char(a.date, 'DDMMYYYY') as date, 'none' as description, pri.wholesale, pri.wholesale_currency,
#           p.master_sku, d.designer, s.season, att.countryoforigin, po.pon,
#           sum( sa.booked + sa.original_surplus ) as quantity, po.type, p.designer_code,
#           p.designer_id, p.season_id, po.purchase_order_id,
#           sup.supplier_code, sup.description as sup_description
#           from purchase_order po, stock_order so, stock_order_item soi, stock_arrived sa, stock_arrived_log sal, product p,
#                 season s, pricing pri, arrival a, designer d, attributes att,
#               variant v, supplier sup
#          where sal.date between ? and ?
#          and sal.stock_action_id = 2
#          and sal.stock_arrived_id = sa.stock_arrived_id
#          and so.purchase_order_id = po.purchase_order_id
#          and (po.type < 3 OR po.type is null)
#          and soi.stock_order_id = so.stock_order_id
#          and soi.stock_order_item_id = sa.stock_order_item_id
#          and sa.arrival_id = a.arrival_id
#          and soi.variant_id = v.variant_id
#          and p.product_id = v.product_id
#          and pri.product_id = p.product_id
#          and p.product_id = att.product_id
#          and p.season_id = s.season_id
#          and p.designer_id = d.designer_id
#          and d.supplier_id = sup.supplier_id
#          group by a.arrival_id, a.date, pri.wholesale, pri.wholesale_currency,
#                     p.master_sku, d.designer, s.season, att.countryoforigin, po.pon, p.designer_code,
#                     p.designer_id, p.season_id, po.purchase_order_id,
#                     sup.supplier_code, sup.description, po.type
#";


my $qry = " 
select distinct del.id as delivery_id, to_char(del.date, 'DDMMYYYY') as date,
'none' as description, pri.wholesale_price, cur.currency,
p.legacy_sku,
d.designer,
s.season,
...
open my $fh, ">", "/opt/xt/deploy/xtracker/script/data_transfer/maxisun/am/dg.csv" || die "Couldn't open file: $!";
