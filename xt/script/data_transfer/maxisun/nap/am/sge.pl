...
oa.country, oa.county, to_char(sisl.date, 'DDMMYYYY') as dispatch_date, r.rma_number, c.currency 
...
open my $fh, ">", "/var/data/xt_static/utilities/data_transfer/maxisun/am/csv/sge.csv" || die "Couldn't open file: $!";
...
foreach my $shipment_id (keys %shipments) {
    ...
    if ($shipments{$shipment_id}{order_nr}) {
        ...
        if ($shipments{$shipment_id}{country} eq "United States") {
            $state = $shipments{$shipment_id}{county};

            if ($state eq "NY") {
                $tax_rates{$shipments{$shipment_id}{country}} = 0.07;
            }
            else {
                $tax_rates{$shipments{$shipment_id}{country}} = 0;
            }
