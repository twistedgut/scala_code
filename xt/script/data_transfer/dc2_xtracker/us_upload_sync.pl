    ...
    eval{
      FIELD:
        foreach my $key ( keys %{ $results_intl_ref->{$pid} } ){
            ...
            # skip long description if product is CITES restricted
            if ($results_us_ref->{$pid}->{cites_restricted} == 1){
                        $skip{long_description} = 1;
                        print "CITES - Skipped Long Desc\n\n";
                }
