...
{
open (my $FILE, '<','/var/data/xt_static/data/spain_invoice_number.txt') || warn "Cannot open file: $!";
while (my $line = <$FILE>) {
    chomp($line);
    if ($line){
        $counter = $line;
...
foreach my $channel_id ( sort {$a <=> $b} keys %channels ) {
    ...
    open (my $CSV,'>','/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_spain_report.csv') || warn "Cannot open csv output file: $!";
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Spain_Invoice.pdf') || die("Unable to create file");
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Spain_Credit_Note.pdf') || die("Unable to create file");
}
