...
from renumeration r LEFT JOIN renumeration_item ri ON r.id =
ri.renumeration_id, link_orders__shipment los, orders o, shipment s,
order_address oa, country c, country_tax_rate ctr,
renumeration_status_log rsl, currency cur
...
foreach my $channel_id ( sort {$a <=> $b} keys %channels ) {
    ...
    open (my $CSV,'>','/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_greece_report.csv') || warn "Cannot open csv output file: $!";
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Greece_Invoice.pdf') || die("Unable to create file");
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Greece_Credit_Note.pdf') || die("Unable to create file");
}
