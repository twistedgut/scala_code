...
ctr.rate))), 2), rsl.date as invoice_date, cur.currency, ler.rate as
fx_rate, r.misc_refund
from renumeration r LEFT JOIN renumeration_item ri ON r.id = ri.renumeration_id, renumeration_status_log rsl, link_orders__shipment los, orders o, shipment s, order_address oa, country c, country_tax_rate ctr, currency cur, local_exchange_rate ler
...
cur.currency, ler.rate, r.misc_refund
order by o.date asc
";

my $qry2 = "
select r.id, o.order_nr, o.date, sum(ri.unit_price) as total_unit_price, sum(ri.tax) as total_tax, case when ctr.rate is not null then r.shipping - (r.shipping * ctr.rate) else r.shipping end, r.shipping * ctr.rate, rsl.date as invoice_date, cur.currency, ler.rate
 as fx_rate, r.misc_refund
from renumeration r LEFT JOIN renumeration_item ri ON r.id = ri.renumeration_id, link_orders__shipment los, orders o, shipment s, order_address oa, country c, country_tax_rate ctr, renumeration_status_log rsl, currency cur, local_exchange_rate ler
...
foreach my $channel_id ( sort {$a <=> $b} keys %channels ) {
    ...
    while(my $row = $sth->fetchrow_arrayref){
        ...
        $csv_output .= "$counter,DEBIT,".$row->[1].','.$row->[7].','.$row->[3].','.$row->[4].','.$row->[5].','.$row->[6].','.$row->[10].','.$row->[8].','.$row->[9]."\r\n";
    ...
    while(my $row = $sth->fetchrow_arrayref){
        ...
        if ($row->[10] > 0 && $row->[10] < 1 ){
            ...
            $csv_output .= "$counter,CREDIT,".$row->[1].','.$row->[7].','.$row->[3].','.$row->[4].','.$row->[5].','.$row->[6].','.$row->[10].','.$row->[8].','.$row->[9]."\r\n";
    ...
    open (my $CSV,'>','/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Lithuania_report.csv') || warn "Cannot open csv output file: $!";
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Lithuania_Invoice.pdf') || die("Unable to create file");
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Lithuania_Credit_Note.pdf') || die("Unable to create file");
}
