...
# get current invoice counter from file
open (my $FILE, '<','/var/data/xt_static/data/bulgaria_invoice_number.txt') || warn "Cannot open file: $!";
while ( my $line = <$FILE>) {
    chomp($line);
    if ($line){
        $counter = $line;
...
# get customer tax numbers from text file
my %tax_numbers = ();
open ($IN,'<','/opt/xt/deploy/xtracker/script/data_transfer/vat_report/country_reports/bulgaria_customer_tax_numbers.txt') || warn "Cannot open file: $!";
while ( my $line = <$IN>) {
    chomp($line);
    my ($customer_nr, $tax_nr) = split(/\t/, $line);

    $tax_numbers{$customer_nr} = $tax_nr;
}
close($IN);
...
ctr.rate))), 2), rsl.date as invoice_date, cur.currency,
cust.is_customer_number, ler.rate as fx_rate, r.misc_refund
from renumeration r LEFT JOIN renumeration_item ri ON r.id = ri.renumeration_id, renumeration_status_log rsl, link_orders__shipment los, orders o, customer cust, shipment s, order_address oa, country c, country_tax_rate ctr, currency cur, local_exchange_rate ler
...
and ler.start_date < rsl.date
and (ler.end_date is null or ler.end_date > rsl.date)
and o.channel_id = ?
group by r.id, o.order_nr, o.date, r.shipping, ctr.rate, rsl.date,
cur.currency, cust.is_customer_number, ler.rate, r.misc_refund
order by o.date asc
";
...
rsl.date as invoice_date, cur.currency, cust.is_customer_number,
ler.rate, r.misc_refund
from renumeration r LEFT JOIN renumeration_item ri ON r.id = ri.renumeration_id, link_orders__shipment los, orders o, customer cust, shipment s, order_address oa, country c, country_tax_rate ctr, renumeration_status_log rsl, currency cur, local_exchange_rate ler
...
my $ch_qry = "select c.id, b.config_section from channel c, business b where c.business_id = b.id and c.name != 'JIMMYCHOO.COM'";
...
foreach my $channel_id ( sort {$a <=> $b} keys %channels ) {
    ...
    while(my $row = $sth->fetchrow_arrayref){
        ...
        generate_invoice( $dbh, $exchange_rate, $row->[0], "IT Department", 1, 1 );

        $invoice_output .= sprintf("%010d", $counter)."<br />";
        $invoice_output .= "EGN Number: ";
        if ($tax_numbers{ $row->[9] }){
            $invoice_output .= $tax_numbers{ $row->[9] }."<br />";
        }
        else {
            $invoice_output .= "999999999<br />";
        }
        ...
        $csv_output .= sprintf("%010d", $counter).",DEBIT,".$row->[1].','.$row->[7].','.$row->[3].','.$row->[4].','.$row->[5].','.$row->[6].','.$row->[11].','.$row->[8].",".$row->[10]."\r\n";
    ...
    while(my $row = $sth->fetchrow_arrayref){
        if ($row->[11] > 0 && $row->[11] < 1 ){
            ...
            $credit_output .= sprintf("%010d", $counter)."<br />";
            $credit_output .= "EGN Number: ";
            if ($tax_numbers{ $row->[9] }){
                $credit_output.= $tax_numbers{ $row->[9] }."<br />";
            }
            else {
                $credit_output .= "999999999<br />";
            }
            ...
            $csv_output .= sprintf("%010d", $counter).",CREDIT,".$row->[1].','.$row->[7].','.$row->[3].','.$row->[4].','.$row->[5].','.$row->[6].','.$row->[11].','.$row->[8].",".$row->[10]."\r\n";
    ...
    open (my $CSV,'>','/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_bulgaria_report.csv') || warn "Cannot open csv output file: $!";
    print $CSV "No.,Type,Order Number,Invoice Date,Total Unit Cost,Total Tax,Shipping,Shipping Tax,Misc,Currency,FX Rate\r\n";
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Bulgaria_Invoice.pdf') || die("Unable to create file");
    ...
    $pdf->to_file('/opt/xt/deploy/xtracker/script/data_transfer/vat_report/output/'.$channels{$channel_id}.'_Bulgaria_Credit_Note.pdf') || die("Unable to create file");
}

# write current counter back to file for next time
open (my $FILE, '>','/var/data/xt_static/data/bulgaria_invoice_number.txt') || warn "Cannot open file: $!";
print $FILE $counter;
close($FILE);
