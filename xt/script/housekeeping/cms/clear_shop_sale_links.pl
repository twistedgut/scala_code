...
and (lower(c.content) like '%shop the sale%' or lower(c.content) like '%/shop/sale/designers/%')
and d.id not in (select designer_id from product where id in (select product_id from product_channel where channel_id = (select id from channel where name = 'NET-A-PORTER.COM') and live = true and visible = true) and id in (select product_id from price_adjustment) group by designer_id)
