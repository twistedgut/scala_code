...
foreach my $shipment (@$shipments_to_cancel) {
    ...
    # just to be sure - this is the range of previously placed test orders
    next if ($order_id < 1823044);
    next if ($order_id > 1843255);
    ...
    next unless ($shipment->link_orders__shipments->first->orders->payments->count == 1
        && $shipment->link_orders__shipments->first->orders->payments->first->settle_ref eq '9999999'); 
