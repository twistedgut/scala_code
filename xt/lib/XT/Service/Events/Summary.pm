package XT::Service::Events::Summary;
...
use XTracker::Config::Local qw(config_section_exists);
...
use Class::Std;
{
    ...
    sub START {
        ...
        $self->check_for_common_config_omissions;

        return;
    }

    sub check_for_common_config_omissions {
        my ($self) = @_;

        foreach my $event_target (qw<OUT-Intl OUT-AM Intl AM>) {
            if (not config_section_exists("Database_pws_schema_${event_target}")) {
                xt_warn(qq{web-db configuration missing for: $event_target});
            }
        }
    ...
    sub process {
        ...
        $handler->{data}{events} = $promotion->events_summary(
               {
                    'detail_websites.website_id'    => { in  => [
                        $PROMOTION_WEBSITE__OUT_DASH_INTL,
                        $PROMOTION_WEBSITE__OUT_DASH_AM,
                    ] },
                }
        );
