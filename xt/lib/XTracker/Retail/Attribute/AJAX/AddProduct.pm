package XTracker::Retail::Attribute::AJAX::AddProduct;
...
sub handler {
        ...
        my $product_sort_preference         = $handler->{param_of}{'product_sort_preference'};
    ...
    # get sales channel id & channel config section from the form
        ...
        if ( $attribute_id && $products && $channel_id ) {
                ...
                if ( @prods ) {
                        ...
                        eval {
                                ...
                                $schema->txn_do( sub {
                                        ...
                                        foreach my $pid (@prods) {
                                                my $failure_reason      = '';
                                                my $success     = $factory->create_product_attribute( {
                                                    'attribute_id'            => $attribute_id,
                                                    'product_id'              => $pid,
                                                    'transfer_dbh_ref'        => $transfer_dbh_ref,
                                                    'operator_id'             => $handler->operator_id,
                                                    'channel_id'              => $channel_id,
                                                    'product_sort_preference' => $product_sort_preference
                                                } );
                                                if ($success) {
                                                        push @val_pids, $pid;
                                                }
                                                else {
                                                        $failure_reason = " - Incompatible Sales Channel";
                                                        $error                  .= "  $pid$failure_reason<br />";
                                                }
                                        }
                        ...
                        if ($@) {
                                ...
                                $error                  = "No Products Were Added<br/><br/>".$@;
                                $no_prods_added = 1;
                        }
        ...
        if ($error) {
                if ( @prods ) {
                        if ( !$no_prods_added ) {
                                $response       = "The following products could not be added:  <br /><br />$error<br />Please check and try again.";
                        }
                        else {
