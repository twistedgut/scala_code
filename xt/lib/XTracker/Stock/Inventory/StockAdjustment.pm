package XTracker::Stock::Inventory::StockAdjustment;
...
sub handler {
    ...
    if( $handler->{data}{variant_id} ){
        ...
        %args                           = ( type => 'variant_id', id => $handler->{data}{variant_id} );
        ...
        %args           = ( type => 'product_id', id => $handler->{data}{product_id} );
    ...
    $handler->{data}{content}       = 'inventory/stock_adjustment.tt';
    $handler->{data}{section}       = 'Stock Control';
    $handler->{data}{subsection}    = 'Inventory';
    $handler->{data}{subsubsection} = 'Adjust Stock';
    ...
    if ( $args{type} ) {
        ...
        # everyone can see the IWS stock - restrictions will be on who can
        # adjust it
        #$args{exclude_iws} = 1 unless $handler->is_manager;
