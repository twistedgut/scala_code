...
BEGIN;
    ...
    -- This one apparently should have been deleted ages ago
    DROP VIEW IF EXISTS "njiv_ftbc_gross_sales_OLD";

    ALTER TABLE orders ALTER COLUMN date SET DATA TYPE TIMESTAMP WITH TIME ZONE USING date AT TIME ZONE 'Europe/London';

    ALTER TABLE shipment ALTER COLUMN date SET DATA TYPE TIMESTAMP WITH TIME ZONE USING date AT TIME ZONE 'Europe/London';
    ALTER TABLE shipment ALTER COLUMN sla_cutoff SET DATA TYPE TIMESTAMP WITH TIME ZONE USING sla_cutoff AT TIME ZONE 'Europe/London';
  ...
  WHERE o.channel_id = 3 AND s.shipment_class_id = 1
  ...
  ORDER BY all_sale.order_nr;
    ...
    JOIN shipment_item si ON s.id = si.shipment_id
    JOIN link_shipment_item__price_adjustment lsipa ON si.id = lsipa.shipment_item_id
   WHERE o.channel_id = 1 AND s.shipment_class_id = 1
   GROUP BY o.date, o.order_nr) all_sale
    JOIN ( SELECT o.date, o.order_nr, count(si.id) AS units
            FROM orders o
       JOIN link_orders__shipment los ON o.id = los.orders_id
