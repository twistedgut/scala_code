...
SELECT name, attribute_type_id, deleted, synonyms, manual_sort, page_id, 5
FROM product.attribute
WHERE attribute_type_id=9 AND channel_id=1 and name not in
    (SELECT name FROM product.attribute WHERE channel_id=5 and attribute_type_id=9);
...
WHERE pnt_nap.attribute_id=pa_nap.id
    AND pa_nap.channel_id=1 AND pa_mrp.channel_id=5
...
WHERE attribute_id IN
    (SELECT id FROM product.attribute WHERE channel_id=5 and attribute_type_id=9)
