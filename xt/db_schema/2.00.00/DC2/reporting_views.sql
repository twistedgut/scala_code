...
WHERE pw.balance = 0 and pw.channel_id = 2
...
(((
	...
	sum(si.unit_price / scr.from_usd) AS merchandise_sales_value,
	sum(si.tax / scr.from_usd) AS tax_value,
	sum(si.duty / scr.from_usd) AS duties_value,
	sum((si.unit_price + si.tax + si.duty) / scr.from_usd) AS gross_sales_value,
	0 AS shipping_value,
	0 AS gift_credit_redeemed,
	0 AS store_credit_redeemed,
	0 AS total,
...
UNION
	...
	0 AS duties_value,
	0 AS gross_sales_value,
	sum(s.shipping_charge / scr.from_usd) AS shipping_value,
	sum(s.gift_credit / scr.from_usd) AS gift_credit_redeemed,
	sum(s.store_credit / scr.from_usd) AS store_credit_redeemed,
	sum((s.shipping_charge + s.gift_credit + s.store_credit) / scr.from_usd) AS total,
	0 AS new_order_count,
	0 AS new_merchandise_sales_value,
	0 AS new_tax_value,
	0 AS new_duties_value,
...
UNION
	...
	sum(si.unit_price / scr.from_usd) AS new_merchandise_sales_value,
	sum(si.tax / scr.from_usd) AS new_tax_value,
	sum(si.duty / scr.from_usd) AS new_duties_value,
	sum((si.unit_price + si.tax + si.duty) / scr.from_usd) AS new_gross_sales_value,
	0 AS new_shipping_value,
	0 AS new_gift_credit_redeemed,
	0 AS new_store_credit_redeemed,
	0 AS new_total
...
UNION
	...
	0 AS new_duties_value,
	0 AS new_gross_sales_value,
	sum(s.shipping_charge / scr.from_usd) AS new_shipping_value,
	sum(s.gift_credit / scr.from_usd) AS new_gift_credit_redeemed,
	sum(s.store_credit / scr.from_usd) AS new_store_credit_redeemed,
	sum((s.shipping_charge + s.gift_credit + s.store_credit) / scr.from_usd) AS new_total
...
WHERE 
scr.destination_currency = 2 AND 'now'::text::date > scr.date_start AND (scr.date_finish IS NULL OR 'now'::text::date < scr.date_finish)
AND o.channel_id = 2
...
WHERE so.type_id = 1 and po.channel_id = 2
...
JOIN log_pws_stock pw ON v.id = pw.variant_id and pw.channel_id = 2
...
WHERE si.variant_id <> 49285 AND si.variant_id <> 67158 AND si.variant_id <> 73652 AND o.channel_id = 2
...
FROM 
	...
	WHERE s.shipment_class_id = 1 AND of.flag_id = 45 AND ssl.shipment_status_id = 4 AND o.channel_id = 2
...
WHERE (ri.return_item_status_id = ANY (ARRAY[5, 6, 7])) AND ri.return_type_id = 1 AND of.flag_id = 45 AND o.channel_id = 2
...
LEFT JOIN purchase_order po ON so.purchase_order_id = po.id AND po.channel_id = 2
...
LEFT JOIN variant v ON p.id = v.product_id
LEFT JOIN log_pws_stock pw ON v.id = pw.variant_id
WHERE pw.balance = 0 and pw.channel_id = 4
GROUP BY p.id;
...
JOIN colour_filter cf ON fcm.filter_colour_id = cf.id
LEFT JOIN product_channel pdc on p.id = pdc.product_id
LEFT JOIN (select product_id,'NAP'::text as onNAP from product_channel where channel_id = 2) nap on p.id = nap.product_id


WHERE scr.destination_currency = 1
AND 'now'::text::date > scr.date_start
AND (scr.date_finish IS NULL OR 'now'::text::date < scr.date_finish)
AND pdc.channel_id = 4
...
JOIN variant v ON p.id = v.product_id 
JOIN log_pws_stock pw ON v.id = pw.variant_id and pw.channel_id = 4
JOIN pws_action pa ON pw.pws_action_id = pa.id; 
...
WHERE si.variant_id <> 49285 AND si.variant_id <> 67158 AND si.variant_id <> 73652 AND o.channel_id = 4
  GROUP BY o.date, date_trunc('day'::text, o.date), o.id, o.order_nr, o.customer_id, cu.is_customer_number, c.currency, oa.country, 
CASE
    WHEN (3 IN ( SELECT order_flag.flag_id
       FROM orders, order_flag
...
FROM 
	...
	WHERE s.shipment_class_id = 1 AND of.flag_id = 45 AND ssl.shipment_status_id = 4 AND o.channel_id = 4
	GROUP BY date_trunc('day'::text, ssl.date), c.currency
...
WHERE (ri.return_item_status_id = ANY (ARRAY[5, 6, 7])) AND ri.return_type_id = 1 AND of.flag_id = 45 AND o.channel_id = 4
...
JOIN stock_order_item soi ON ldi_soi.stock_order_item_id = soi.id
JOIN stock_order so ON so.id = soi.stock_order_id
LEFT JOIN purchase_order po ON so.purchase_order_id = po.id AND po.channel_id = 4
JOIN variant v ON soi.variant_id = v.id
