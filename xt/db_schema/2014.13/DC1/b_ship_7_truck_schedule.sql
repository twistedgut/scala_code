BEGIN;
    ...
    INSERT INTO sos.truck_departure__shipment_class (truck_departure_id, shipment_class_id)
        SELECT td.id, (SELECT id FROM sos.shipment_class WHERE api_code = 'NOMDAY' )
            FROM sos.truck_departure td
            JOIN sos.week_day wd ON td.week_day_id = wd.id
            WHERE carrier_id = (SELECT id FROM sos.carrier WHERE code = 'DHL')
            AND wd.name IN ('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday');
    ...
    INSERT INTO sos.truck_departure__shipment_class (truck_departure_id, shipment_class_id)
        SELECT id, (SELECT id FROM sos.shipment_class WHERE api_code = 'PREMIER_DAYTIME' )
            ...
            WHERE carrier_id = (
                ...
                WHERE code = 'NAP'
                AND departure_time IN ('09:30:00', '11:45:00')
            );
    INSERT INTO sos.truck_departure__shipment_class (truck_departure_id, shipment_class_id)
    ...
    INSERT INTO sos.truck_departure__shipment_class (truck_departure_id, shipment_class_id)
        SELECT id, (SELECT id FROM sos.shipment_class WHERE api_code = 'PREMIER_ALL_DAY' )
            ...
            WHERE carrier_id = (
                SELECT id FROM sos.carrier
                WHERE code = 'NAP'
                AND departure_time IN ('09:30:00', '11:45:00', '15:45:00')
            );
