/*******************************************************************************
* Product Data Audit Logging
* Modification for field-level audit-logging
*
* TjG, October 2007
*******************************************************************************/
...
/*
################################################################################
## AUTOGENERATED DATABASE PATCH - DO NOT EDIT THIS FILE                       ##
##                                                                            ##
## Generated by: [finalise path]/generate_audit_trigger.pl                    ##
################################################################################
*/

---------------------------------------------------------------------------
-- Changes to existing audit schema
---------------------------------------------------------------------------
SELECT 'Changes to existing audit.product table...';

BEGIN;

ALTER TABLE audit.product ADD field_name varchar(50);
ALTER TABLE audit.product ADD value_pre text;
ALTER TABLE audit.product ADD value_post text;

COMMIT;


---------------------------------------------------------------------------
-- Create CAST: boolean => text
---------------------------------------------------------------------------
BEGIN;

CREATE OR REPLACE FUNCTION bool_to_text (boolean) RETURNS TEXT
...
CREATE CAST (boolean as text) WITH FUNCTION bool_to_text(boolean);
...
---------------------------------------------------------------------------
SELECT 'Modify audit.product_trigger()...';

BEGIN;

CREATE OR REPLACE FUNCTION audit.product_trigger() RETURNS trigger AS '
DECLARE
    -- Variables
...
BEGIN
	...
	ELSIF (TG_RELNAME = ''product'') THEN
		...
		IF (TG_OP = ''INSERT'') THEN
			...
			INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''visible'', NULL, CAST(NEW.visible AS Text), v_product_id, v_action_id, v_comment);
		ELSIF (TG_OP = ''UPDATE'') THEN
			IF ( COALESCE(CAST(NEW.visible AS TEXT), '''') <> COALESCE(CAST(OLD.visible AS TEXT), '''') ) THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''visible'', CAST(OLD.visible AS Text), CAST(NEW.visible AS Text), v_product_id, v_action_id, v_comment);
			END IF;
		ELSIF (TG_OP = ''DELETE'') THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''visible'', CAST(OLD.visible AS Text), NULL, v_product_id, v_action_id, v_comment);
		END IF;
		IF (TG_OP = ''INSERT'') THEN
		...
		IF (TG_OP = ''INSERT'') THEN
			...
			INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''live'', NULL, CAST(NEW.live AS Text), v_product_id, v_action_id, v_comment);
		ELSIF (TG_OP = ''UPDATE'') THEN
			IF ( COALESCE(CAST(NEW.live AS TEXT), '''') <> COALESCE(CAST(OLD.live AS TEXT), '''') ) THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''live'', CAST(OLD.live AS Text), CAST(NEW.live AS Text), v_product_id, v_action_id, v_comment);
			END IF;
		ELSIF (TG_OP = ''DELETE'') THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''live'', CAST(OLD.live AS Text), NULL, v_product_id, v_action_id, v_comment);
		END IF;
		IF (TG_OP = ''INSERT'') THEN
		...
		IF (TG_OP = ''INSERT'') THEN
			...
			INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''staging'', NULL, CAST(NEW.staging AS Text), v_product_id, v_action_id, v_comment);
		ELSIF (TG_OP = ''UPDATE'') THEN
			IF ( COALESCE(CAST(NEW.staging AS TEXT), '''') <> COALESCE(CAST(OLD.staging AS TEXT), '''') ) THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''staging'', CAST(OLD.staging AS Text), CAST(NEW.staging AS Text), v_product_id, v_action_id, v_comment);
			END IF;
		ELSIF (TG_OP = ''DELETE'') THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''staging'', CAST(OLD.staging AS Text), NULL, v_product_id, v_action_id, v_comment);
		END IF;
		IF (TG_OP = ''INSERT'') THEN
		...
		IF (TG_OP = ''INSERT'') THEN
			...
			INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''disableupdate'', NULL, CAST(NEW.disableupdate AS Text), v_product_id, v_action_id, v_comment);
		ELSIF (TG_OP = ''UPDATE'') THEN
			IF ( COALESCE(CAST(NEW.disableupdate AS TEXT), '''') <> COALESCE(CAST(OLD.disableupdate AS TEXT), '''') ) THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''disableupdate'', CAST(OLD.disableupdate AS Text), CAST(NEW.disableupdate AS Text), v_product_id, v_action_id, v_comment);
			END IF;
		ELSIF (TG_OP = ''DELETE'') THEN
				INSERT INTO audit.product (operator_id, table_id, pushed_to_live, field_name, value_pre, value_post, product_id, action_id, comment) VALUES (v_operator_id, v_table_id, v_pushed_to_live, ''disableupdate'', CAST(OLD.disableupdate AS Text), NULL, v_product_id, v_action_id, v_comment);
		END IF;
		IF (TG_OP = ''INSERT'') THEN
...
-- N.B.: Uncomment the following section for execution on goblin (PostgreSQL version 8.2.1)
/*
BEGIN;

SELECT 'Adding audit trigger to product';
DROP TRIGGER IF EXISTS product_tgr ON product;
...
DROP TRIGGER IF EXISTS product_tgr ON product_attribute;
CREATE TRIGGER product_tgr AFTER INSERT OR UPDATE OR DELETE
ON product_attribute
    FOR EACH ROW EXECUTE PROCEDURE audit.product_trigger();

SELECT 'Adding audit trigger to price_country';
DROP TRIGGER IF EXISTS product_tgr ON price_country;
CREATE TRIGGER product_tgr AFTER INSERT OR UPDATE OR DELETE
ON price_country
    FOR EACH ROW EXECUTE PROCEDURE audit.product_trigger();

SELECT 'Adding audit trigger to price_default';
DROP TRIGGER IF EXISTS product_tgr ON price_default;
CREATE TRIGGER product_tgr AFTER INSERT OR UPDATE OR DELETE
ON price_default
    FOR EACH ROW EXECUTE PROCEDURE audit.product_trigger();

SELECT 'Adding audit trigger to price_region';
DROP TRIGGER IF EXISTS product_tgr ON price_region;
CREATE TRIGGER product_tgr AFTER INSERT OR UPDATE OR DELETE
ON price_region
    FOR EACH ROW EXECUTE PROCEDURE audit.product_trigger();

SELECT 'Adding audit trigger to shipping_attribute';
DROP TRIGGER IF EXISTS product_tgr ON shipping_attribute;
CREATE TRIGGER product_tgr AFTER INSERT OR UPDATE OR DELETE
