...
INSERT INTO ship_restriction_location (ship_restriction_id, location, type) (
    SELECT (SELECT id FROM ship_restriction WHERE title = 'CITES'), code, 'COUNTRY' FROM country WHERE code != '' AND sub_region_id != (SELECT id FROM sub_region WHERE sub_region = 'EU Member States')
);
