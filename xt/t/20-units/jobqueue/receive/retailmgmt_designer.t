...
for my $designer_name (
    'Teest',
    "Téèst",
    # "T\x{e9}\x{e8}st", # TODO this should work but is apparently broken until we get perl 5.16 :( -- speak to dakkar for details
    "T\N{U+00E9}\N{U+00E8}st",
    "T✪st",
    "T\N{U+272A}st",
) {
    _test_designer( $schema, $fake_job, $designer_name );
}
_test_designer_category( $schema, $fake_job );
...
sub _test_designer {
    ...
    $schema->txn_do( sub {
        ...
        SKIP: {
            skip 'test is broken - consumer behaviour needs fixing for duplicate designer additions', 2;
        ...
        $payload = {
            ...
            designer_id         => '99423',
            designer_name       => $designer_name,
            designer_name_new   => "$designer_name 2",
            url_key             => "Teest_2",
            supplier_code       => '5678',
        ...
        is($designer_rs->count, 1, 'Still just the one designer found in DB');
        $designer = $designer_rs->first;
        is($designer->designer, $payload->{designer_name_new}, 'designer name updated OK');
        is($designer->url_key, $payload->{url_key}, 'url_key updated OK');
...
sub _test_designer_category {
    ...
    my $designer1 = $designer_rs->find_or_create({
        id => ($max_designer_id + 1),
        designer => 'test des '.($max_designer_id + 1),
        url_key => 'test1',
    });

    my $designer1_channel_details = {
    ...
    my $designer1_channel = $designer1
        ->search_related('designer_channel',$designer1_channel_details)
        ->count;

    $designer1->create_related('designer_channel',$designer1_channel_details)
        unless $designer1_channel;

    my $designer2 = $designer_rs->find_or_create({
        id => ($max_designer_id + 2),
        designer => 'test des '.($max_designer_id + 2),
        url_key => 'test2',
    });

    my $designer2_channel_details = {
        channel_id => $channel->id,
        page_id => undef,
        website_state_id => $DESIGNER_WEBSITE_STATE__INVISIBLE,
        description => 'foo2',
    ...
    # Search if it exists already
    my $designer2_channel = $designer2
        ->search_related('designer_channel',$designer2_channel_details)
        ->count;

    $designer2->create_related('designer_channel',$designer2_channel_details)
        unless $designer2_channel;
...
sub _get_channels {
    my $schema = shift;
    return map { $_->id } $schema->resultset('Public::Channel')->search({'is_enabled'=>1},{order_by => 'id'})->all;
}
