package Test::XTracker::Order::Actions::ChangeShipmentItemStatus;
...
Test::XTracker::Order::Actions::ChangeShipmentItemStatus

=head1 DESCRIPTION

Tests the 'XTracker::Order::Actions::ChangeShipmentItemStatus' class. A mocked up
...
use XTracker::Order::Actions::ChangeShipmentItemStatus  ();
...
use XTracker::Constants::FromDB     qw( :customer_issue_type );
...
=head2 test_basket_update_when_cancelling_items

For Orders paid using a Payment Method that requires the PSP to be notified of changes,
this tests that when using the '/.*/.*/ChangeShipmentItemStatus' Handler to Cancel a
Shipment Item, results in the Order Value being reduced and that the PSP is notified.
...
sub test_basket_update_when_cancelling_items : Tests {
    my $self = shift;
    ...
    # with five products this should add up to 410
    my $payment_amount      = 390;
    ...
    my @shipment_items = $shipment->shipment_items->order_by_sku->all;
    ...
    note "Cancel a Shipment Item so there is still a need for a Payment - with Payment that doesn't require PSP to be updated";
    ...
    $self->_cancel_shipment_item_and_check_ok( $shipment, $shipment_items[0] );
    ...
    note "Cancel a Shipment Item so there is still a need for a Payment - with Payment that DOES require PSP to be updated";
    ...
    $self->_cancel_shipment_item_and_check_ok( $shipment, @shipment_items[1,2] );
    ...
    note "Cancel a Shipment Item so that now Store Credit covers the Value of the Order";
    $mock_lwp->clear_all->add_response_OK( $psp_success_response );

    $self->_cancel_shipment_item_and_check_ok( $shipment, $shipment_items[3] );
...
# helper that will Cancel an Item for a Shipment & check it worked
sub _cancel_shipment_item_and_check_ok {
    my ( $self, $shipment, @items_to_cancel ) = @_;

    my $card_debit_tender = $shipment->order->card_debit_tender;
    my $original_value    = $card_debit_tender->value;

    my $total_item_value = 0;
    my %cancel_item_args;
    foreach my $item ( @items_to_cancel ) {
        my $item_id = $item->id;

        $total_item_value += $item->purchase_price;

        $cancel_item_args{ "item-${item_id}" }   = 1;
        $cancel_item_args{ "reason-${item_id}" } = $CUSTOMER_ISSUE_TYPE__8__OTHER;
    }
    ...
    # Cancel the Item
    XTracker::Order::Actions::ChangeShipmentItemStatus::handler( $mock_web_layer );

    foreach my $item ( @items_to_cancel ) {
        my $is_cancelled = $item->discard_changes->is_cancelled || $item->is_cancel_pending;
        cmp_ok( $is_cancelled, '==', 1, "Item '" . $item->id . "' has been Cancelled" );
    }

    is(
        format_currency_2dp( $card_debit_tender->discard_changes->value ),
        format_currency_2dp( ( $original_value - $total_item_value ) ),
...
# helper to populate the GET Params of a Cancel Item
...
sub _get_mock_web_layer_using_default_payload {
    ...
    return Test::XTracker::Mock::WebServerLayer->setup_mock_with_get_params(
        '/CustomerCare/OrderSearch/ChangeShipmentItemStatus',
        \%get_params,
    );
}
