...
fulfilment_pigeonhole_multiple.t - Pack multiple items from pigeon holes (IWS only)

=head1 DESCRIPTION

Pack multiple items from pigeon holes (IWS only).

Add and delete comments on the Packing Exception page.

#TAGS fulfilment packing packingexception iws http checkruncondition duplication whm

=head1 SEE ALSO

fulfilment_pigeonhole_mixed.t

=cut

use FindBin::libs;

# Test packing of multiple items with pigeon holes (only used in DC1 with IWS).
