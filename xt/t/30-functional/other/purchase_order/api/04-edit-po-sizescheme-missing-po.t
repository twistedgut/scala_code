...
04-edit-po-sizescheme-missing-po.t - tests that a size scheme change is not possible if all purchase orders not sent
...
Attempt to make a size scheme change via the REST API, expects the update to
fail as we have not provided all purchase orders for a give product whose
size scheme we are attempting to change.
...
$mech->content_like(qr/The purchase_order: .* is in XT but not in data from fulcrum \(\d+\), update not possible/);
